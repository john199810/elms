...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
        ...
        $stmt = $conn->prepare("SELECT username, password FROM employees WHERE employee_id = ?");
        ...
        $update_query = "UPDATE employees SET username = ?, password = ? WHERE employee_id = ?";
        $stmt_update = $conn->prepare($update_query);
        $stmt_update->bind_param("ssi", $username, $hashed_password, $employee_id);
        ...
        Swal.fire({
            ...
            if (result.isConfirmed || result.isDismissed) {
                window.location.href = 'employee_dashboard.php'; // Adjust the redirect URL as needed
            }
        });
        </script>";
        } else {
